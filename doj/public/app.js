// Public reader — gesetz.bloodline.cc
// Tabs: Gesetzbuch (with diff highlights + propose) | Änderungen (changelog)
...
const state = {
  ...
  rechnerSel: [],     // ausgewaehlte Delikte fuer den Bussgeldrechner
  rechnerQuery: "",   // Suchfeld im Rechner
};

const CATEGORY_ORDER = ["Verfassung","Strafrecht","Verfahren","Verkehr","Verwaltung"];
const HIDDEN_CATEGORIES = new Set(["Katalog"]);
const HIDDEN_SLUGS = new Set(["bussgeldkatalog", "los-santos-tabelle", "schadensersatz", "schadensersatztabelle"]);
...
// Bußgeldkatalog live aus der DB laden (überschreibt die statische bussgeldkatalog.js).
// Fällt bei Fehler still auf die statische Datei zurück.
async function loadCatalog() {
  try {
    const { catalog } = await api("/api/catalog");
    if (Array.isArray(catalog) && catalog.length) window.BUSSGELDKATALOG = catalog;
  } catch { /* statische Datei bleibt Fallback */ }
}
...
function render() {
  ...
  else if (state.tab === "katalog") renderKatalogView();
  else if (state.tab === "rechner") renderRechnerView();
...
function renderLawsView() {
  ...
  const sidebar = `
    <aside class="sidebar">
      <input class="search" id="search" type="text" placeholder="Gesetz suchen…" value="${escapeHtml(state.filter)}" autocomplete="off" />
      ${orderedCats
        .map(
          (cat) => `
      ...
      `
        ...
        .join("")}
      <button class="btn btn-gold" id="propose-new" style="width:100%;margin-top:14px">+ Neues Gesetz vorschlagen</button>
    </aside>
  `;
  ...
  let article;
  if (!law) {
    article = `<article class="article"><div class="placeholder">Wähle ein Gesetz aus dem Verzeichnis links.</div></article>`;
  } else {
    ...
    const pendingBanner = pending.length
      ? `<div class="proposal-banner">
           ...
           <div class="proposal-banner-text">
             ...
             <small>Eingereicht von ${pending.map((p) => escapeHtml(p.proposerName)).join(", ")} — wird vom DOJ geprüft.</small>
           </div>
         </div>`
      : "";
    article = `
  ...
  enhanceLawTOCAnchors(main, (state.laws.find((l) => l.slug === state.current) || {}).title || "");
}
...
function enhanceLawTOCAnchors(root, lawTitle) {
  ...
  // 3. Doc-eigene TOC-Linklisten (Google-Docs Bookmark-Listen) entfernen.
  //    Diese waren oft in mehrere <ol> zersplittert -> doppeltes/dreifaches
  //    Inhaltsverzeichnis. Wir bauen unten EIN sauberes TOC aus den Überschriften.
  if (!body.querySelector(".toc-grid")) {
    body.querySelectorAll("ol, ul").forEach((list) => {
      if (list.closest(".toc-grid, .toc-list")) return;
      const lis = [...list.querySelectorAll(":scope > li")];
      if (lis.length < 3) return;
      const linkish = lis.filter((li) => {
        const a = li.querySelector("a[href]");
        if (!a) return false;
        const href = a.getAttribute("href") || "";
        return href.startsWith("#") || /#_?heading/i.test(href);
      }).length;
      if (linkish / lis.length >= 0.6) list.remove();
    });
  }
  ...
  // 4b. EIN kanonisches Inhaltsverzeichnis aus den Section-Überschriften bauen.
  //     Erkennt "§ 1 Gültigkeitsbereich", "Artikel 1", "Präambel" usw. Badge = laufende
  //     Nummer (1..N, lückenlos, keine Dubletten); die §/Artikel-Nummer bleibt im Label
  //     erhalten, damit die Zuordnung eindeutig ist.
  if (!body.querySelector(".toc-grid, .toc-list")) {
    const isHeadingLike = (t) => {
      if (!t || t.length > 70) return false;
      if (/^§\s*\d*\s*$/.test(t)) return false;     // leeres "§" / "§ " ohne Titel
      if (/^\d+$/.test(t)) return false;            // reine Zahl
      if (/^\(\d+/.test(t)) return false;           // "(2) …" = Absatz, kein Titel
      if (/[.;:]$/.test(t)) return false;           // endet wie ein Satz
      return t.split(/\s+/).length <= 9;            // Titel sind kurz
    };
    // Label = Titel ohne "§ N"-Prefix (Zahl steht schon im Badge). "Artikel N" bleibt.
    const toLabel = (t) => {
      const stripped = t.replace(/^§\s*\d+[a-z]?\s+/i, "").trim();
      return stripped || t;
    };
    // Folgt einer Überschrift direkt ein fetter Untertitel (z.B. GG:
    // "Artikel N" + <p><strong>Titel</strong></p>), hängen wir den Titel an,
    // damit im Inhaltsverzeichnis der echte Titel steht statt nur "Artikel N".
    const subtitleFor = (h) => {
      const nx = h.nextElementSibling;
      if (!nx || nx.tagName !== "P") return "";
      const st = nx.querySelector("strong, b");
      if (!st) return "";
      if (nx.textContent.trim() !== st.textContent.trim()) return "";
      const sub = st.textContent.trim();
      return sub.length <= 80 ? sub : "";
    };
    // Dokumenttitel ("… [GG]", Code-Klammer am Ende) und Kapitel-Trenner
    // ("I. Die Grundrechte") gehören nicht ins Inhaltsverzeichnis — nur die
    // echten Abschnitte (§/Artikel). Sektions-Prefixe wie "[StGb] …"
    // (Klammer am Anfang, z.B. Bußgeldkatalog) bleiben erhalten.
    // Titel normalisieren: [CODE], "Los Santos" und Satzzeichen weg -> Kern vergleichbar.
    const titleKey = (s) => (s || "")
      .replace(/\[[^\]]*\]/g, "")
      .replace(/\blos santos\b/ig, "")
      .toLowerCase().replace(/[^a-z0-9äöüß]+/g, " ").trim();
    const docKey = titleKey(lawTitle);
    const isDocTitle = (t) =>
      /\[[A-Za-zÄÖÜäöüß]{1,10}\]\s*$/.test(t) ||           // "… [GG]"
      (docKey && titleKey(t) === docKey);                   // = Gesetzestitel (auch ohne Klammer)
    const isChapterDivider = (t) => /^[IVXLCDM]+\.\s/.test(t);
    const heads = [...body.querySelectorAll("h2, h3, h4")]
      .map((h) => ({ h, base: toLabel((h.textContent || "").trim()) }))
      .filter((x) => isHeadingLike((x.base || "").trim()))
      .filter((x) => !isDocTitle(x.base.trim()) && !isChapterDivider(x.base.trim()))
      .map(({ h, base }) => {
        const sub = subtitleFor(h);
        return { id: h.id, label: sub ? `${base} — ${sub}` : base };
      });
    ...
    if (heads.length >= 3) {
      ...
      nav.innerHTML = heads.map((h, idx) => `
        <a class="toc-list-item" href="#${h.id}">
          <span class="toc-list-num">${idx + 1}</span>
          <span class="toc-list-label">${escapeHtml(h.label)}</span>
  ...
  // 4c. Sicherheitsnetz: falls trotzdem mehrere TOC-Blöcke entstanden -> nur den ersten behalten
  const navs = [...body.querySelectorAll(".toc-grid, .toc-list")];
  ...
  body.querySelectorAll('a[href^="#"]').forEach((a) => {
    a.addEventListener("click", (ev) => {
      ...
      void target.offsetWidth; // Reflow -> Animation auch bei erneutem Sprung auf dieselbe Section
      target.classList.add("flash-target");
      setTimeout(() => target.classList.remove("flash-target"), 2200);
    });
  });
}
...
// Zahlenbereich aus Text lesen: "$18.000 - $30.000" -> {min:18000,max:30000},
// "$600" -> {600,600}, "-" -> {0,0}. Punkt = Tausender.
function _range(s) {
  const toks = String(s ?? "").match(/[\d.]+/g);
  if (!toks) return { min: 0, max: 0 };
  const vals = toks.map((t) => parseInt(t.replace(/\./g, ""), 10)).filter((n) => !isNaN(n));
  if (!vals.length) return { min: 0, max: 0 };
  return { min: Math.min(...vals), max: Math.max(...vals) };
}
function _fmtMoney(min, max) {
  const f = (n) => "$" + n.toLocaleString("de-DE");
  return min === max ? f(min) : `${f(min)} – ${f(max)}`;
}
function _fmtHe(min, max) {
  if (min === 0 && max === 0) return "— HE";
  return (min === max ? `${min}` : `${min} – ${max}`) + " HE";
}
...
function renderRechnerView() {
  ...
  const totGMin = sel.reduce((s, i) => s + i.gMin, 0);
  const totGMax = sel.reduce((s, i) => s + i.gMax, 0);
  const totHMin = sel.reduce((s, i) => s + i.hMin, 0);
  const totHMax = sel.reduce((s, i) => s + i.hMax, 0);
  ...
  const results = q
    ? matches.length
      ? matches
          .map(
            (e) => `
        <button class="rch-result rechner-add" data-par="${escapeHtml(e.paragraph)}" data-bed="${escapeHtml(e.bedeutung)}" data-gmin="${e.gMin}" data-gmax="${e.gMax}" data-hmin="${e.hMin}" data-hmax="${e.hMax}">
    ...
    ? sel
      ...
      <li class="rch-sel-item">
        ...
        <span class="rch-sel-vals">
          <span class="rch-geld">${_fmtMoney(e.gMin, e.gMax)}</span>
          <span class="rch-he">${_fmtHe(e.hMin, e.hMax)}</span>
    ...
    <div class="article-body">
      <div class="rch-grid">
        ...
        <aside class="rch-cart">
          ...
          <div class="rch-total">
            <div class="rch-total-row"><span>Gesamtstrafe</span><span class="rch-total-geld">${_fmtMoney(totGMin, totGMax)}</span></div>
            <div class="rch-total-row"><span>Hafteinheiten</span><span class="rch-total-he">${_fmtHe(totHMin, totHMax)}</span></div>
          </div>
        </aside>
      </div>
    </div>
    ...
    <div class="changelog-shell">
      <div class="changelog-card">
        <header class="changelog-head">
          <h2>Änderungs­historie</h2>
          <p>Alle Anpassungen am Gesetzbuch, die durch Vorschläge der Community freigegeben wurden. Kürzlich geänderte Passagen sind im Gesetz selbst goldfarben hervorgehoben.</p>
        </header>
        ${stats}
        ${list}
      </div>
...
function renderProposeModal() {
  ...
  const initialCat = state.proposeKind === "edit" ? (law.category || "Verwaltung") : "Verwaltung";

  wrap.innerHTML = `
    <div class="modal" style="max-width:1100px">
      <div class="modal-head">
        <h3>${state.proposeKind === "edit" ? `Änderung vorschlagen: ${escapeHtml(law.title)}` : "Neues Gesetz vorschlagen"}</h3>
        <button class="btn btn-ghost" id="close-prop">✕</button>
      </div>
      <div class="modal-body">
        <p class="notice notice-info">Dein Vorschlag wird vom DOJ geprüft. Erst nach manueller Freigabe durch einen Mitarbeiter wird er übernommen. Bis dahin sieht das DOJ deinen Vorschlag separat hervorgehoben.</p>
...
document.addEventListener("click", (e) => {
  ...
  if (rAdd) {
    ...
    state.rechnerSel.push({
      ...
      gMin: parseInt(rAdd.dataset.gmin, 10) || 0,
      gMax: parseInt(rAdd.dataset.gmax, 10) || 0,
      hMin: parseInt(rAdd.dataset.hmin, 10) || 0,
      hMax: parseInt(rAdd.dataset.hmax, 10) || 0,
    });
    render();
    return;
  }
...
(async function init() {
  await Promise.all([loadLaws(), loadPendingProposals(), loadCatalog()]);
