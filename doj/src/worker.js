...
async function listCatalog(env) {
  const { results: secs } = await env.DB.prepare(
    `SELECT code, title, position FROM catalog_sections ORDER BY position ASC, title ASC`
  ).all();
  const { results: ents } = await env.DB.prepare(
    `SELECT id, code, paragraph, bedeutung, strafe, he, sonstiges, position
     FROM catalog_entries ORDER BY position ASC, id ASC`
  ).all();
  const byCode = {};
  (secs || []).forEach((s) => {
    byCode[s.code] = { code: s.code, title: s.title, entries: [] };
  });
  (ents || []).forEach((e) => {
    const sec = byCode[e.code];
    if (!sec) return;
    sec.entries.push({
      id: e.id,
  ...
  return (secs || []).map((s) => byCode[s.code]);
}
...
async function handleApi(req, env, url, user) {
  ...
  // Public read
  if (p === "/api/laws" && m === "GET") {
    return json({ laws: await listLawsPublic(env) });
  }

  if (p === "/api/changelog" && m === "GET") {
    return json(await listChangelog(env));
  }

  // Public read: Bußgeldkatalog (für Katalog-Ansicht + Rechner)
  if (p === "/api/catalog" && m === "GET") {
    return json({ catalog: await listCatalog(env) });
  }
  ...
  // Bot-Login: Discord-Bot mintet eine Admin-Session. Geschützt durch
  // geteiltes Secret (env.BOT_SECRET) zwischen Bot und Worker.
  ...
  // ---------- Bußgeldkatalog CRUD (Editor + Admin) ----------

  // Sektion anlegen
  if (p === "/api/admin/catalog/sections" && m === "POST") {
    const body = await req.json().catch(() => ({}));
    const code = String(body.code || "").trim().slice(0, 20);
    const title = String(body.title || "").trim().slice(0, 120);
    if (!code || !title) return err(400, "Code und Titel erforderlich");
    const exists = await env.DB.prepare(`SELECT 1 FROM catalog_sections WHERE code = ?`).bind(code).first();
    if (exists) return err(409, "Code bereits vergeben");
    const max = await env.DB.prepare(`SELECT COALESCE(MAX(position), -1) AS m FROM catalog_sections`).first();
    await env.DB.prepare(
      `INSERT INTO catalog_sections (code, title, position) VALUES (?, ?, ?)`
    ).bind(code, title, (max?.m ?? -1) + 1).run();
    await audit(env, user.username, "catalog.section.create", code, title);
    return json({ ok: true });
  }

  const secMatch = p.match(/^\/api\/admin\/catalog\/sections\/([^/]+)$/);
  if (secMatch && m === "PUT") {
    const code = decodeURIComponent(secMatch[1]);
    const body = await req.json().catch(() => ({}));
    const title = String(body.title || "").trim().slice(0, 120);
    if (!title) return err(400, "Titel erforderlich");
    const res = await env.DB.prepare(`UPDATE catalog_sections SET title = ? WHERE code = ?`).bind(title, code).run();
    if (res.meta.changes === 0) return err(404, "Sektion nicht gefunden");
    await audit(env, user.username, "catalog.section.update", code, title);
    return json({ ok: true });
  }

  if (secMatch && m === "DELETE") {
    if (user.role !== "admin") return err(403, "Admin only");
    const code = decodeURIComponent(secMatch[1]);
    await env.DB.prepare(`DELETE FROM catalog_entries WHERE code = ?`).bind(code).run();
    await env.DB.prepare(`DELETE FROM catalog_sections WHERE code = ?`).bind(code).run();
    await audit(env, user.username, "catalog.section.delete", code, null);
    return json({ ok: true });
  }

  // Eintrag anlegen
  if (p === "/api/admin/catalog/entries" && m === "POST") {
    const body = await req.json().catch(() => ({}));
    const code = String(body.code || "").trim();
    const bedeutung = String(body.bedeutung || "").trim().slice(0, 400);
    if (!code || !bedeutung) return err(400, "Sektion und Bedeutung erforderlich");
    const sec = await env.DB.prepare(`SELECT 1 FROM catalog_sections WHERE code = ?`).bind(code).first();
    if (!sec) return err(404, "Sektion nicht gefunden");
    const paragraph = String(body.paragraph || "").trim().slice(0, 60);
    const strafe = String(body.strafe || "").trim().slice(0, 80);
    const he = String(body.he || "").trim().slice(0, 40);
    const sonstiges = String(body.sonstiges || "").trim().slice(0, 500);
    const max = await env.DB.prepare(`SELECT COALESCE(MAX(position), -1) AS m FROM catalog_entries WHERE code = ?`).bind(code).first();
    const r = await env.DB.prepare(
      `INSERT INTO catalog_entries (code, paragraph, bedeutung, strafe, he, sonstiges, position) VALUES (?, ?, ?, ?, ?, ?, ?)`
    ).bind(code, paragraph || null, bedeutung, strafe || null, he || null, sonstiges || null, (max?.m ?? -1) + 1).run();
    await audit(env, user.username, "catalog.entry.create", code, bedeutung);
    return json({ ok: true, id: r.meta.last_row_id });
  }

  const entMatch = p.match(/^\/api\/admin\/catalog\/entries\/(\d+)$/);
  if (entMatch && m === "PUT") {
    const id = Number(entMatch[1]);
    const body = await req.json().catch(() => ({}));
    const bedeutung = String(body.bedeutung || "").trim().slice(0, 400);
    if (!bedeutung) return err(400, "Bedeutung erforderlich");
    const paragraph = String(body.paragraph || "").trim().slice(0, 60);
    const strafe = String(body.strafe || "").trim().slice(0, 80);
    const he = String(body.he || "").trim().slice(0, 40);
    const sonstiges = String(body.sonstiges || "").trim().slice(0, 500);
    const res = await env.DB.prepare(
      `UPDATE catalog_entries SET paragraph = ?, bedeutung = ?, strafe = ?, he = ?, sonstiges = ? WHERE id = ?`
    ).bind(paragraph || null, bedeutung, strafe || null, he || null, sonstiges || null, id).run();
    if (res.meta.changes === 0) return err(404, "Eintrag nicht gefunden");
    await audit(env, user.username, "catalog.entry.update", String(id), bedeutung);
    return json({ ok: true });
  }

  if (entMatch && m === "DELETE") {
    const id = Number(entMatch[1]);
    await env.DB.prepare(`DELETE FROM catalog_entries WHERE id = ?`).bind(id).run();
    await audit(env, user.username, "catalog.entry.delete", String(id), null);
    return json({ ok: true });
  }
...
export default {
  async fetch(req, env, ctx) {
    ...
    const isAdmin = host === env.ADMIN_HOST || host.startsWith("doj.");
    const isPublic = host === env.PUBLIC_HOST || host.startsWith("gesetz.");
    ...
    if (url.pathname.startsWith("/api/")) {
      ...
      const publicAllowed = new Set([
        "/api/laws", "/api/me", "/api/changelog", "/api/catalog", "/api/proposals", "/api/pending-proposals",
      ]);
    ...
    if (url.pathname === "/auth/login") {
      ...
      return new Response(null, {
        ...
        headers: {
          location: authUrl,
          "set-cookie": `doj_state=${state}; Path=/; Max-Age=600; HttpOnly; Secure; SameSite=Lax`,
        },
      });
    }
    ...
    if (url.pathname === "/auth/callback") {
      try {
        ...
        const cookieState = (req.headers.get("cookie") || "").match(/(?:^|;\s*)doj_state=([^;]+)/);
        ...
        // Zugang, wenn mindestens EINE der berechtigten Rollen vorhanden ist.
        const allowed = String(env.DISCORD_ROLE_IDS || env.DISCORD_ROLE_ID || "")
          .split(",").map((s) => s.trim()).filter(Boolean);
        const hasRole = Array.isArray(roles) && allowed.some((r) => roles.includes(r));
        if (!hasRole) {
    ...
    // Bot-Magic-Login: setzt Session-Cookie aus gültigem Token, leitet zur Startseite
