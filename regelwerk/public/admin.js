...
const CATEGORIES = [
  "Allgemeine Regeln", "Roleplay Regeln", "Fraktionsregelwerk",
];
  ...
  return `
    <header class="topbar">
      <div class="topbar-inner">
        <a class="brand" href="#">
          <img src="https://bloodline.cc/img/bloodline/bl_transparent.png" alt="Bloodline" />
          <span class="brand-copy">
            <span class="brand-eyebrow">Verwaltung</span>
            <span class="brand-title">Regelwerk · Verwaltung</span>
          </span>
        </a>
        <nav>
          <button data-nav="laws" class="${state.view === "laws" ? "active" : ""}">Regeln</button>
          ${isAdmin ? `<button data-nav="users" class="${state.view === "users" ? "active" : ""}">Benutzer</button>` : ""}
          <button data-nav="account" class="${state.view === "account" ? "active" : ""}">${escapeHtml(state.user.displayName || state.user.username)}</button>
  ...
  return `
    <div class="auth-wrap">
      <div class="auth-card">
        <div class="auth-head">
          <img src="https://bloodline.cc/img/bloodline/bl_transparent.png" alt="Bloodline" />
          <h1>Regelwerk · Verwaltung</h1>
          <p>Editor-Portal · regelwerk.bloodline.cc</p>
        </div>
        <div class="auth-body">
          ${(() => {
            ...
            if (!err) return "";
            const map = {
              norole: "Kein Zugang — dir fehlt die Rolle <strong>Highteam</strong>.",
              oauth_unconfigured: "Discord-Login ist noch nicht eingerichtet. Nutze solange <code>/regelwerk-login</code> im Discord.",
              state: "Sitzung abgelaufen — bitte erneut versuchen.",
              token: "Discord-Anmeldung fehlgeschlagen.",
              norole_user: "Kein Zugang.",
            };
          ...
          <div class="discord-login">
            <p class="discord-login-intro">Nur Mitglieder mit der Rolle <strong>Highteam</strong> erhalten Zugang.</p>
...
function renderRequestForm() {
  return `
    ...
    <form id="request-form">
      ...
      <div class="field"><label>Begründung / Rolle im Team</label><textarea name="reason" placeholder="z.B. Admin, Supporter, Fraktionsverwaltung…" style="min-height:90px"></textarea></div>
  ...
  const sidebar = `
    <aside class="sidebar">
      <div style="display:flex;gap:6px;margin-bottom:10px">
        ...
        <button class="btn btn-gold" id="new-law" title="Neue Regel">+</button>
      </div>
      ${orderedCats
        .map(
          (cat) => `
    ...
    <div class="modal-bg" id="prop-bg">
      <div class="modal" style="max-width:1200px">
        <div class="modal-head">
          <h3>${escapeHtml({edit:"Änderungs­vorschlag",new:"Neue Regel",delete:"Löschungs­vorschlag"}[p.kind])}: ${escapeHtml(p.proposedTitle || p.slug || "—")}</h3>
          ...
          <tbody>
            ${state.users
              .map(
                (u) => `
              <tr>
                <td><strong>${escapeHtml(u.displayName || u.username)}</strong></td>
...
function renderAccount() {
  return `
    ...
    <main class="shell" style="max-width:600px">
      ...
      <div class="card">
        <p><strong>Benutzer:</strong> ${escapeHtml(state.user.displayName || state.user.username)} <span class="tag ${state.user.role === 'admin' ? 'admin' : ''}">${escapeHtml(state.user.role)}</span></p>
...
function render() {
  // Scroll-Positionen merken, damit sie beim Re-Render nicht zurückspringen
  const sb = document.querySelector(".sidebar");
  const sbTop = sb ? sb.scrollTop : 0;
  const ab = document.querySelector(".article-body");
  const abTop = ab ? ab.scrollTop : 0;
  ...
  if (sb2) sb2.scrollTop = sbTop;
  const ab2 = document.querySelector(".article-body");
  if (ab2) ab2.scrollTop = abTop;
